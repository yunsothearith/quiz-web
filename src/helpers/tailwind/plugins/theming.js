        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--helpers-${e(key)}`, value], [`--helpers-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--helpers-${e(key)}`, value], [`--helpers-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                },
            };
        });
